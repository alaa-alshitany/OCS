    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        predictBtn.setOnClickListener {
            ...
            hklRef.getFile(localFile).addOnSuccessListener {
                ...
                val mutInputTensor = TensorBuffer.createFixedSize(intArrayOf(1, 1, 34673, 1), DataType.FLOAT32)
                ...
                val expInputTensor = TensorBuffer.createFixedSize(intArrayOf(1, 697), DataType.FLOAT32)
                ...
                val methyInputTensor = TensorBuffer.createFixedSize(intArrayOf(1, 808), DataType.FLOAT32)
                ...
                val drugInputTensor = TensorBuffer.createFixedSize(intArrayOf(1, 1, 1), DataType.FLOAT32)
                ...
                FirebaseModelDownloader.getInstance().getModel(
                    "Regrission", DownloadType.LOCAL_MODEL_UPDATE_IN_BACKGROUND, conditions).addOnSuccessListener {
                        ...
                        if (modelFile != null) {
                            ...
                            for (i in 0..4) {
                                Log.d("input tensor $i shape", interpreter.getInputTensor(i).shape().contentToString())
                                Log.d("input tensor $i data type", interpreter.getInputTensor(i).dataType().name)
                                Log.d("input tensor $i name", interpreter.getInputTensor(i).name())
                            }
                            Log.d("output tensor shape", interpreter.getOutputTensor(0).shape().contentToString()
                            )
                            Log.d("output tensor data type", interpreter.getOutputTensor(0).dataType().name)
                            ...
                            if (outputBinary == 1) {
                                Log.d("Drug is Resistance for patient with percentage:  ",
                                    "${roundedOutput * 100} %"
                                )
                            } else {
                                Log.d("Drug is Sensitive for patient with percentage:  ",
                                    "${roundedOutput * 100} %"
                                )
                            }
